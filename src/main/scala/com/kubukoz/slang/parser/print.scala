...
def prettyPrint(fileName: String, source: String, e: Parser.Error): String =
  ...
  val location = lm.toLineCol(e.failedAtOffset).fold(s"offset ${e.failedAtOffset}") { case (line, column) =>
    s"""line $line, column $column""".stripMargin
  }
  ...
  val allCode = s"""$codeBefore😡 $codeAfter"""
    .linesWithSeparators
    .zipWithIndex
    .map { case (line, index) =>
      s"""%${lineCountLength}d | $line""".format(index + 1)
    }
    .mkString
